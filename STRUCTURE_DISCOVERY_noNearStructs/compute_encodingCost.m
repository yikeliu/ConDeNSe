function [ MDLcost ] = compute_encodingCost( subgraph, N_tot, n_sub, E, n_sub2, nb_edges, ca, alpha)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   alpha: optional - power law exponent of 'hb'
%  ---------------                                                        %
%  OUTPUTS:                                                               %
...
switch subgraph
    ...
    case 'hb'
        edges_inc = nnz(E);
        edges_exc = size(E,1)*size(E,2)-nnz(E);
        if edges_inc == 0
            MDLcost = LR( alpha ) + LN( n_sub ) + l2cnk(N_tot, n_sub) + log2( n_sub^2 );
        else
            MDLcost = LR( alpha ) + LN( n_sub ) + l2cnk(N_tot, n_sub) + ... %log2(nchoosek(vpi(N_tot), n_sub)) + ...
                log2( n_sub^2 ) + edges_inc * NLL( edges_inc, edges_exc, 1) + ...
                edges_exc * NLL( edges_inc, edges_exc, 0);
        end
    case 'err'
