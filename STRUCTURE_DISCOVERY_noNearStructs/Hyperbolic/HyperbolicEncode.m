...
for i = 1:length(groups)
	if str2num(commMembership{i,1})<=minSize
		continue;
	else
	groupNum = i;
	%trueCompNodes = find(groups == groupNum)';
	group_temp = groups{i,2};
	group_temp = str2num(group_temp);
	trueCompNodes = group_temp; 
    % plot degree distribution of groups
    %Asub = AOrig(trueCompNodes, trueCompNodes);
    %groupFre = tabulate(sum(Asub));
    %unqDeg = groupFre(:, 1);
    %disp(unqDeg);
    %degCount = groupFre(:, 2);
    %disp(degCount);
    %scatter(unqDeg, degCount);
    %xlabel('degree');
    %ylabel('count');
    %print(['degree_distribution_', num2str(i)], '-dpng');
...
copyfile('model.mat','../../DATA/Hyperbolic');
