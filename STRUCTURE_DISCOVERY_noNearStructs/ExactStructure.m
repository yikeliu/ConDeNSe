...
if ( m == n*n - n )      % full clique
	...
	edge_all = nnz(AOrig(node_all,node_all));
        %edge_all = edge_all(:);
        % compute quality
        MDLcost_fc_perf = compute_encodingCost( 'fc', N_tot, n, zeros(n,n));
        qual = MDLcost_fc_perf/MDLcost_fc;
        model(model_idx) = struct('code', 'fc', 'edges', edge_all, 'nodes1', top_gccind(curind), 'nodes2', [], 'benefit', costGain, 'benefit_notEnc', costGain_notEnc, 'quality', qual);
	...
	edge_all = nnz(AOrig(node_all,node_all));
        %edge_all = edge_all(:);
        % compute quality
        MDLcost_ch_perf = compute_encodingCost( 'ch', N_tot, n, zeros(n,n));
        qual = MDLcost_ch_perf/MDLcost_ch;
        model(model_idx) = struct('code', 'ch', 'edges', edge_all, 'nodes1', top_gccind(curind), 'nodes2', [], 'benefit', costGain, 'benefit_notEnc', costGain_notEnc, 'quality', qual);
        %entries = size(model, 2);
...
elseif (m == 2*(n-1))   % chain or star
    ...
    if d1count == 2 && d2count == n-2     % chain
        ...
        model_idx = model_idx + 1;
        % edges in the model
        node_all = sort([curind(d1ind(1)),curind(d2ind(1:size(d2ind, 2))),curind(d1ind(2))]);
        %edge_all = zeros(size(AOrig));
        %edge_all(node_all,node_all) = AOrig(node_all,node_all);
        %edge_all = nnz(edge_all);
	edge_all = nnz(AOrig(node_all,node_all));
        ...
        MDLcost_ch_perf = compute_encodingCost( 'ch', N_tot, n, zeros(n,n));
        qual = MDLcost_ch_perf/MDLcost_ch;
        model(model_idx) = struct('code', 'ch', 'edges', edge_all, 'nodes1', [top_gccind(curind(d1ind(1))) top_gccind(curind(d2ind(1:size(d2ind, 2)))) top_gccind(curind(d1ind(2)))], 'nodes2', [], 'benefit', costGain, 'benefit_notEnc', costGain_notEnc, 'quality', qual);
        %entries = size(model, 2);
        %model(entries+1) = struct('code', 'ch', 'nodes1', top_gccind(curind), 'nodes2', [], 'benefit', costGain);
    ...
    if ( max(evals) == - min(evals) )  % bipartite graph (special case: star)
        ...
        if length(set1) == 1 && length(set2) == 1
            ...
            model_idx = model_idx + 1;
            % edges in the model
            node_all = sort(curind([set1, set2]));
            %edge_all = zeros(size(AOrig));
            %edge_all(node_all,node_all) = AOrig(node_all,node_all);
            %edge_all = nnz(edge_all);
	    edge_all = nnz(AOrig(node_all,node_all));
            ...
            MDLcost_ch_perf = compute_encodingCost( 'ch', N_tot, n, zeros(n,n));
            qual = MDLcost_ch_perf/MDLcost_ch;
            model(model_idx) = struct('code', 'ch', 'edges', edge_all, 'nodes1', top_gccind(curind([set1, set2])), 'nodes2', [], 'benefit', costGain, 'benefit_notEnc', costGain_notEnc, 'quality', qual);
	    ...
	    edge_all = nnz(AOrig(node_all,node_all));
            %edge_all = edge_all(:);
            % compute quality
            MDLcost_st_perf = compute_encodingCost( 'st', N_tot, n, zeros(n,n));
            qual = MDLcost_st_perf/MDLcost_st;
            model(model_idx) = struct('code', 'fc', 'edges', edge_all, 'nodes1', top_gccind(curind), 'nodes2', [], 'benefit', costGain, 'benefit_notEnc', costGain_notEnc, 'quality', qual);
            %entries = size(model, 2);
        ...
        elseif length(set2) == 1
            ...
            model_idx = model_idx + 1;
            % edges in the model
            node_all = sort([curind(set1),curind(set1)]);
            %edge_all = zeros(size(AOrig));
            %edge_all(node_all,node_all) = AOrig(node_all,node_all);
            %edge_all = nnz(edge_all);
	    edge_all = nnz(AOrig(node_all,node_all));
            ...
            MDLcost_st_perf = compute_encodingCost( 'st', N_tot, n, zeros(n,n));
            qual = MDLcost_st_perf/MDLcost_st;
            model(model_idx) = struct('code', 'st', 'edges', edge_all, 'nodes1', top_gccind(curind(set1)), 'nodes2', top_gccind(curind(set1)), 'benefit', costGain, 'benefit_notEnc', costGain_notEnc, 'quality', qual);
            %entries = size(model, 2);
	        ...
	        edge_all = nnz(AOrig(node_all,node_all));
                %edge_all = edge_all(:);
                % compute quality
                MDLcost_bc_perf = compute_encodingCost( 'bc', N_tot, length(set1), zeros(n,n), length(set2));
                qual = MDLcost_bc_perf/MDLcost_bc;
                model(model_idx) = struct('code', 'bc', 'edges', edge_all, 'nodes1', top_gccind(curind(set1)), 'nodes2', top_gccind(curind(set2)), 'benefit', costGain, 'benefit_notEnc', costGain_notEnc, 'quality', qual);
            else
		    ...
		    edge_all = nnz(AOrig(node_all,node_all));
                    %edge_all = edge_all(:);
                    % compute quality
                    MDLcost_nb_perf = compute_encodingCost( 'nb', N_tot, length(set1), zeros(n,n), length(set2));
                    qual = MDLcost_nb_perf/MDLcost_nb;
                    model(model_idx) = struct('code', 'nb', 'edges', edge_all, 'nodes1', top_gccind(curind(set1)), 'nodes2', top_gccind(curind(set2)), 'benefit', costGain, 'benefit_notEnc', costGain_notEnc, 'quality', qual);
                end
            end
