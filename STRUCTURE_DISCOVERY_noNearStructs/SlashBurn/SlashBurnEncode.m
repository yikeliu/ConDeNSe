...
%     disp(topind);
    ...
    for i=1:size(star_cores, 2)
        ...
        % check the clusters given by slashburn for duplicates
        clusIdOld = clusId;
        clusId = sort([cur_center, satellites]);
        %clusNum = clusNum + 1;
        if isequal(clusId, clusIdOld)
                continue;
        end
        %disp('Failed to drop the cluster!!!');
	...
	top_gccind = sort([gccind(cur_center), gccind(satellites)]);
        ...
        EncodeSubgraph(AOrig(top_gccind,top_gccind), curind, top_gccind, n, out_fid, info, minSize);
...
%             % edges in the model
%             node_all = sort([cur_center,satellites]);
%             %disp(node_all);
...
%             %edge_all = edge_all(:);
%             % compute quality
%             MDLcostFC_perf = compute_encodingCost( 'st', n, n_star, zeros(n_star, n_star));
%             qual = MDLcostFC_perf/MDLcostST;
%             model(model_idx) = struct('code', 'st', 'edges', edge_all, 'nodes1', gccind(cur_center), 'nodes2', gccind(satellites), 'benefit', costGain, 'benefit_notEnc', costGain_notEnc, 'quality', qual);
...
%             top_gccind = sort([gccind(cur_center), gccind(satellites)]);
%             curind = 1:size(top_gccind,2);
%             MDLcostFC_perf = compute_encodingCost( 'fc', n, n_star, zeros(n_star, n_star));
%             
%             switch minIdx
%                 case 1
...
copyfile('model.mat','../../DATA/SlashBurn');
