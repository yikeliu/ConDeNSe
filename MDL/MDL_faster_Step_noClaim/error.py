...
class Error :
	...
	# add overlapped edges
	for x in self.overlappedLast:
	    (i,j,k) = x;
	    self.delOverlappedError(i,j);
    ...
    def delOverlappedError(self, i, j) :
	self.overlapped[min(i,j)-1].remove(max(i,j)-1);
        self.numEdgeOverlapped -= 1;
        return;

    # keep track of the changes for overlapped edges
    def updateOverlappedLast(self, i, j, k) :
        self.overlappedLast.append((i,j,k)); 
