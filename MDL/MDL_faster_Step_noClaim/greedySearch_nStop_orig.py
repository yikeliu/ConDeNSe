...
if len(sys.argv) > 2 and sys.argv[2][0] != '-' :
    ...
    mFilenameGreedy = 'optGreedySelection_nStop_' + optMethod + '_' + mFilename_main;
    fgreedy = open(mFilenameGreedy,'w')
    mFilenameGreedyCost = 'optGreedySelection_costs_' + optMethod + '_' + mFilename_main;
    ...
    while times <= min(structLim, maxStructs) :  # add upto structLim structures or as many as there are in the model file  
       ...
       for s in structs_not_selected:
           ...
           if toKeep == 'false':
               ...
               #structs_to_remove.append(s);
           print "%.0f :" % s + " l_total_x %.0f" % l_total_x + "\tl_total_x_best %.0f" % l_total_x_best + "\tl_total_best %.0f" % l_total_best + "\tl_total_wCoverage %.0f" % l_total_wCoverage + "\tl_total_wNEWcoverage %.0f" % l_total_wNEWcoverage;
           print "M_x:\t" + '%.0f' % l_total_x + "\t" + '%.0f' % l_model_x + "\t" + '%.0f' % l_error_x + "\t" + str(E_x.numModellingErrors) + '/' + str(E_x.numCellsCovered) + '\t' + str(E_x.numUnmodelledErrors)  + '/' + str(((E_x.numNodes * E_x.numNodes)-E_x.numNodes) - E_x.numCellsCovered) + '\t\t' + str(E_x.numCellsExcluded) + '\t\t%.0f' % l_total_wCoverage + '\t%.0f' % l_total_wNEWcoverage  + "\t\t%.0f" % l_total_coverageRepeatedErr + "\t%.0f" % l_total_coverageRepeatedAll;
	       ...
	       if optMethod == "simple":
	           ...
	           print "%.0f :" % s + " l_total_x %.0f" % l_total_x + "\tl_total_best %.0f" % l_total_x_best + "\tl_total_best %.0f" % l_total_best + "\tl_total_check %.0f" % l_total_check + "\tl_total_wCoverage %.0f" % l_total_wCoverage + "\tl_total_wNEWcoverage %.0f" % l_total_wNEWcoverage;
	           print "M_x:\t" + '%.0f' % l_total_x + "\t" + '%.0f' % l_model_x + "\t" + '%.0f' % l_error_x + "\t" + str(E_x.numModellingErrors) + '/' + str(E_x.numCellsCovered) + '\t' + str(E_x.numUnmodelledErrors)  + '/' + str(((E_x.numNodes * E_x.numNodes)-E_x.numNodes) - E_x.numCellsCovered) + '\t\t' + str(E_x.numCellsExcluded) + '\t\t%.0f' % l_total_wCoverage + '\t%.0f' % l_total_wNEWcoverage  + "\t\t%.0f" % l_total_coverageRepeatedErr + "\t%.0f" % l_total_coverageRepeatedAll;
       ...
       if times == 10 or times % 200 == 0 :
           mFilenameGreedyTemp = 'optGreedySelection_' + optMethod + '_' + str(times) + '_' + mFilename_main;
    ...
    #output final cost
    target = open('out_final_opt.out','w');
    ...
    #targer.write("M_x:\t" + '%.0f' % l_total_prev + "\t" + '%.0f' % l_model_x + "\t" + '%.0f' % l_error_prev + "\t" + str(E_x_old.numModellingErrors) + '/' + str(E_x_old.numCellsCovered) + '\t' + str(E_x_old.numUnmodelledErrors)  + '/' + str(((E_x_old.numNodes * E_x_old.numNodes)-E_x_old.numNodes) - E_x_old.numCellsCovered) + '\t\t' + str(E_x_old.numCellsExcluded) + '\t\t%.0f' % l_total_wCoverage + '\t%.0f' % l_total_wNEWcoverage  + "\t\t%.0f" % l_total_coverageRepeatedErr + "\t%.0f" % l_total_coverageRepeatedAll;)
    #target.close();
